...
async function verify() {
    ...
    if (!authStore.twoFactorToken) {
        error.value = 'Two-factor session expired. Please sign in again.';
        return;
    }
    if (!code.value) {
        error.value = 'Enter your verification code.';
        return;
    }
    loading.value = true;
    try {
        ...
        // Token may be one-shot / expired — send back to login on hard failures
...
<template>
    <div class="mt-sm-13 mt-8">
        <v-switch v-model="useRecovery" color="primary" hide-details class="mb-4" label="Use a recovery code instead" />

        <template v-if="!useRecovery">
            <v-label class="text-subtitle-1 font-weight-semibold pb-2 text-lightText">Type your 6 digit code</v-label>
        ...
        <template v-else>
            <v-label class="text-subtitle-1 font-weight-semibold pb-2 text-lightText">Recovery code</v-label>
            <VTextField v-model="recoveryCode" hide-details="auto" class="mb-2" autocomplete="one-time-code" />
        </template>

        <v-btn color="primary" size="large" block flat class="mt-4" :loading="loading" @click="verify">Verify</v-btn>

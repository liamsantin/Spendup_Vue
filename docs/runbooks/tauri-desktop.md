...
1. Copier `.env.example` → `.env` ; renseigner `VITE_API_BASE_URL`, `VITE_GOOGLE_CLIENT_ID`, `VITE_GOOGLE_DESKTOP_CLIENT_ID`.
2. API : audience Desktop Client ID + CORS (`http://localhost:5173` en dev).
3. Google Cloud : client OAuth **Desktop** (pas d’URI à saisir — loopback `127.0.0.1` géré par l’app).

...
const filteredEmails = computed(() => {
    return selectedFilter.value ? store.emails.filter((email) => email[selectedFilter.value as keyof EmailType]) : store.emails;
});
...
<template>
    ...
    <v-card elevation="10">
        <AppEmailCard>
            ...
            <template v-slot:mailList>
                ...
                <EmailListing :emails="filteredEmails" class="d-md-none d-block" @click="isMobileDrawerOpen = !isMobileDrawerOpen" />
            </template>
    ...
    <!-- Drawer for Email Detail mobile view -->
    <v-navigation-drawer v-if="!lgAndUp" v-model="isMobileEmailCategory" location="left" temporary width="350">
        <v-card-text class="pa-6">
            <EmailCompose @filter-change="handleFilterChange" />
        </v-card-text>
    </v-navigation-drawer>
